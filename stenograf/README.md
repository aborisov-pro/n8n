...
| HTTP Header Auth (Янекс СпичКит) | `Authorization: Api-Key <key>` для distance-распознавания | Submit to SpeechKit, Check Operation Status |
...
## Нормализация транскриптов

После получения `merged_transcript` из СпичКит текст требует нормализации — ASR выдаёт сырые фрагменты с артефактами разбивки и без пунктуации. Ниже описан воспроизводимый пайплайн обработки с помощью больших языковых моделей.

### Этапы

**Инвентаризация** — перед обработкой проверяется, что для лекции заполнены `merged_transcript` и `segmented_at`. Если `segmented_at IS NULL`, сначала прогоняется `fn_segment_lecture`.

**Нормализация текста** — применяется в три фазы:

▪ **Фаза 1 — слипание обрывков.** Строки ≤ 2 слов приклеиваются к следующей нормальной строке; все внутренние заглавные буквы после пробела опускаются в строчные (артефакт ASR, не проп. существительные).

▪ **Фаза 2 — пунктуация.** Граница предложения ставится только при сильных сигналах: следующая единица начинается с маркера-перехода (`так`, `давайте`, `теперь`, `итак`, `сейчас`, `значит`…) или длина накопленного предложения превышает 320 символов. Иначе — запятая, следующая единица идёт со строчной. Результат — длинные разговорные предложения с пунктуацией.

▪ **Фаза 3 — абзацы.** Разрыв при накоплении > 650 символов предложений.

Параллельно собирается **глоссарий ASR-искажений** под предметную область (паттерны `пауэрквэри` → `Power Query`, `дакс` → `DAX` и т.п.), сортируемый по частоте.

**Ревью классификации** — выявление артефактов записи: очень короткие блоки в конце лекции (разрядка), `tool_overview` в середине (склейка аудио), хвостовые блоки в начале следующей лекции.

**Нормализованный MD** — итоговый документ с разделами по `topic_key`, якорями для оглавления и пометками `⚠`/`ℹ` на аномальных блоках.

### Типовая последовательность вызовов

1. `execute_sql` — инвентаризация корпуса (список лекций, сегментов, `topic_key`)
2. `execute_sql` — выгрузка через `jsonb_agg` → результат в файле
3. Python: сборка MD v1 (сырой)
4. Python: нормализация → MD v2 + глоссарий
5. Python: MD v3 с подтаксономией и пометками
6. `apply_migration` × 3–5 (DDL + DML)
7. `execute_sql` × 3–4 верификационных запроса

### Что универсально, что меняется

Универсальны: SQL-шаблоны, алгоритм нормализации (фазы 1–3, пороги), логика работы с generated columns и UNIQUE constraints, DROP+CREATE для view при изменении колонок, список маркеров-переходов русскоязычного лектора.

Меняются под каждую тему: глоссарий ASR-искажений, подтаксономия demo-блоков, список артефактов записи, порядок этапов в MD.
